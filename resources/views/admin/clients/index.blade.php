                        ...
                        <table class="min-w-full bg-white">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">رقم
                                        الهاتف</th>
                                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">البريد
                                        الإلكتروني</th>
                                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">
                                        الاشتراكات النشطة</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($clients as $client)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" dir="ltr"
                                            class="text-right">{{ $client->phone ?? '-' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $client->email }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            @php
                                                $activeSubs = $client->subscriptions->where('status', 'active')->where('ends_at', '>', now());
                                            @endphp
                                                ...
                                                <div class="flex flex-col gap-1">
                                                    @foreach($activeSubs as $sub)
                                                        <span
                                                            ...
                                                            {{ $sub->target_language ?? 'General' }}
                                                            <span class="mx-1 text-gray-400">|</span>
                                                            {{ ucfirst(str_replace('_', ' ', $sub->plan_type)) }}
                                                        </span>
                                                    @endforeach
                                                </div>
                                            @else
                                                <span class="text-gray-400 italic text-xs">لا يوجد اشتراك نشط</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $client->created_at->translatedFormat('d M, Y') }}</td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center text-gray-500">لا يوجد عملاء مسجلون
                                            حالياً.</td>
                                    </tr>
                                @endforelse
                            </tbody>
        ...
        <div x-show="openModal" x-transition
            ...
            <div @click.away="openModal = false" class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
                <div class="text-center">
                    ...
                    <p class="text-sm text-gray-500 mt-2">هل أنت متأكد من رغبتك في حذف حساب <strong
                            x-text="clientName"></strong>؟</p>
                    <div class="mt-6 flex justify-center gap-4">
                        <form :action="'{{ url('admin/clients') }}/' + clientId" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="px-4 py-2 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700">نعم،
                                قم بالحذف</button>
                        </form>
                        <button @click="openModal = false"
                            class="px-4 py-2 bg-white text-gray-700 font-semibold rounded-lg border border-gray-300 hover:bg-gray-50">إلغاء</button>
                    </div>
                </div>
            </div>
        </div>
