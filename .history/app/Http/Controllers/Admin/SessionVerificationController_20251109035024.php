...
class SessionVerificationController extends Controller
{
    ...
    $pendingSessions = Appointment::whereIn('status', ['completed', 'conflict', 'verified', 'rejected', 'pending_verification']) // Added 'pending_verification' from your controller
    ...
    public function verify(Appointment $appointment)
    {
        ...
        if ($appointment->status === 'verified' || $appointment->status === 'cancelled') { // Added 'cancelled' to prevent re-processing
