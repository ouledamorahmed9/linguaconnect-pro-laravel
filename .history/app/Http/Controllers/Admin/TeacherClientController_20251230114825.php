...
public function toggle(Request $request, User $teacher)
    {
        ...
        // 1. Check strict Eligibility (Active Status + Future Date)
        // We REMOVED the 'lessons_used' check so you can link new students easily.
        ...
        if ($validated['checked']) {
            // If trying to link, ensure they have a subscription
            if (!$isEligible) {
                return response()->json([
                    'status' => 'error',
                    'message' => 'Client has no active subscription.'
                ], 422);
            }

            // LINK: Create the row in client_teacher table
            $teacher->clients()->syncWithoutDetaching([$client->id]);
            return response()->json(['status' => 'attached']);
        } else {
            // UNLINK: Remove the row
