...
class SessionVerificationController extends Controller
{
    ...
    {
    // OLD QUERY:
    // $pendingSessions = Appointment::whereIn('status', ['completed', 'conflict'])
    //     ->with(['teacher', 'client'])->get();

    // NEW QUERY (eager-load the report):
    $pendingSessions = Appointment::whereIn('status', ['completed', 'conflict', 'verified', 'rejected', 'pending_verification'])
        ->with(['teacher', 'client', 'meetReport']) // <-- ADD 'meetReport'
        ->orderBy('start_time', 'desc')
        ->paginate(10); // <-- THIS IS THE PAGINATION FIX (was ->get())
    ...
    public function verify(Appointment $appointment)
    {
        ...
        // We will allow 'completed' (from teacher) and 'pending_verification' (from API)
        if (!in_array($appointment->status, ['pending_verification', 'completed', 'conflict'])) {
        ...
        try {
            ...
            DB::transaction(function () use ($appointment) {
                ...
                // We check if it's 'completed' OR 'pending_verification' (which implies completion)
