...
public function edit(Request $request, User $teacher)
    {
        ...
        // 1. Get IDs of clients ALREADY assigned to this teacher (for the checkboxes)
        $assignedClientIds = $teacher->clients()->pluck('users.id')->toArray();

        // 2. Fetch ALL clients who have an active subscription
        // We removed the 'lessons_used' check to ensure they appear even if they haven't started learning yet.
        $eligibleClients = User::where('role', 'client')
            ->whereHas('subscriptions', function ($query) {
                $query->where('status', 'active')
                      ->whereDate('ends_at', '>', now()); // strict date check
            })
            ...
            ->orderBy('name')
            ->paginate(50) // Increased pagination to see more users
