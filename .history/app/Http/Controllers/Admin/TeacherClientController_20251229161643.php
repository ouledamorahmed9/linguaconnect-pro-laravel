...
public function toggle(Request $request, User $teacher)
    {
        ...
        // We removed the 'lessons_used' check to ensure linking works even for new students.
        ...
        if ($validated['checked']) {
            ...
            // This adds the student to the 'client_teacher' pivot table
            $teacher->clients()->syncWithoutDetaching([$client->id]);
            return response()->json(['status' => 'attached', 'message' => 'Client assigned.']);

        } else {
            // This removes the student from the 'client_teacher' pivot table
