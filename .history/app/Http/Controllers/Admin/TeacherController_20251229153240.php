...
public function edit(Request $request, User $teacher)
    {
        ...
        // CLEAN QUERY: We only check if they are a client and have an active, future-dated subscription.
        $eligibleClients = User::where('role', 'client')
            ->whereHas('subscriptions', function ($query) {
                $query->where('status', 'active')
                      ->where('ends_at', '>', now());
                // We removed 'whereColumn' here to ensure new students with 0 lessons always show up.
            })
            ...
            ->orderBy('name')
            ->get(); // Using get() instead of paginate() so all students load in your scrollable list.
