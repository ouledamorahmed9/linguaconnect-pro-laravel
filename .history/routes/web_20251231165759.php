...
Route::middleware(['auth', 'verified'])->get('/dashboard', function () {
    ...
    if ($user->hasRole('teacher')) {
        return redirect()->route('teacher.dashboard'); // Ensure you have this route name
    }

    if ($user->hasRole('client')) {
        return redirect()->route('client.dashboard'); // We created this one earlier
    }
...
Route::get('/ref/{code}', function ($code) {
    // 1. Find the user who owns this code
    $referrer = User::where('referral_code', $code)->first();

    // 2. If valid, queue a cookie for 30 days (43200 minutes)
    if ($referrer) {
...
Route::get('/subscriptions/create', [SubscriptionController::class, 'create'])->name('subscriptions.create');
    Route::post('/subscriptions', [SubscriptionController::class, 'store'])->name('subscriptions.store');    Route::delete('/subscriptions/{subscription}', [AdminSubscriptionController::class, 'destroy'])->name('subscriptions.destroy');
