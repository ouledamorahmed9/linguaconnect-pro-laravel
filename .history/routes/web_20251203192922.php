...
// NEW: log per appointment, not per weekly slot
    // Session Logging Routes (per appointment)
    Route::get('/sessions/{appointment}/log', [SessionLogController::class, 'create'])
        ->name('sessions.log.create');

    Route::post('/sessions/{appointment}/log', [SessionLogController::class, 'store'])
        ->name('sessions.log.store');
});
