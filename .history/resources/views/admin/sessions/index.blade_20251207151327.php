<x-app-layout>
    ...
    <div class="py-12" x-data="{ reportModalOpen: false, reportData: null, rawExtensionData: null, debugText: '' }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- Debug area (for troubleshooting) --}}
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex items-center justify-end space-x-3 rtl:space-x-reverse">
                                                    ...
                                                    <x-secondary-button
                                                        ...
                                                        class="text-xs"
                                                        x-on:click="
                                                            reportData = @json($session->reportData ?? null);
                                                            rawExtensionData = @json($session->rawExtensionData ?? null);
                                                            debugText = 'clicked';
        ...
        <template x-if="reportModalOpen">
            <div class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50" x-transition>
                <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg relative" x-transition.scale>
                    ...
                    <template x-if="reportData && Object.keys(reportData).length">
                        <div>
                            ...
                            <table class="w-full border text-sm">
                                ...
                                <tbody>
                                    <template x-if="reportData.participants && reportData.participants.length">
                                        <template x-for="(participant, idx) in reportData.participants" :key="idx">
                                            <tr class="hover:bg-gray-50">
                                                <td class="border px-2 py-1" x-text="participant.name ?? 'N/A'"></td>
                                                <td class="border px-2 py-1" x-text="participant.firstSeen ?? 'N/A'"></td>
                                                <td class="border px-2 py-1" x-text="participant.timeInCall ?? 'N/A'"></td>
                                            </tr>
                                        </template>
                                    </template>
                                    <template x-if="!reportData.participants || !reportData.participants.length">
                                        <tr><td colspan="3" class="border px-2 py-2 text-center text-gray-500">لا توجد بيانات مشاركين</td></tr>
                                    </template>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <template x-if="(!reportData || !Object.keys(reportData).length) && rawExtensionData">
                        <div class="text-sm text-gray-600">
                            <p class="font-semibold mb-1">لم يتم فك التشفير أو البيانات ليست JSON صالحة.</p>
                            <pre class="text-xs bg-gray-100 rounded-md p-3 overflow-x-auto" x-text="rawExtensionData"></pre>
                        </div>
                    </template>

                    <template x-if="!reportData && !rawExtensionData">
                        <p class="text-sm text-gray-500">لا توجد بيانات تقرير.</p>
                    </template>

                    <div class="text-center mt-6">
                        <button @click="reportModalOpen = false" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                            إغلاق
                        </button>
                    </div>
                </div>
