<x-app-layout>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">مدار بواسطة</th>
                                        ...
                                        <tr @if($session->client->coordinator) class="bg-red-50" @endif>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                @if($session->client->coordinator)
                                                    ...
                                                    <div class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">منسق</div>
                                                @else
                                                    <div class="font-medium text-indigo-600">Admin</div>
                                                    <div class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-indigo-100 text-indigo-800">المنصة</div>
                                                @endif
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex items-center justify-end space-x-3 rtl:space-x-reverse">
                                                    ...
                                                    <x-secondary-button
                                                        ...
                                                        x-on:click="sessionData = JSON.parse(atob('{{ base64_encode($session->extension_data) }}')); reportModalOpen = true;">
                                                        عرض التقرير
                                                    </x-secondary-button>
                                                    @endif
                                                    ...
                                                    <form method="POST" action="{{ route('admin.sessions.dispute', $session) }}" class="inline-block" onsubmit="return confirm('هل أنت متأكد من إرسال هذه الحصة للنزاعات؟');">
                                                        @csrf
    ...
    <template x-if="reportModalOpen">
        <div class_removed="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50" x-transition>
            <div class_removed="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg relative" x-transition.scale>
                ...
                <div class="text-center mt-6">
                    <button @click="reportModalOpen = false" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">إغلاق</button>
                </div>
            </div>
        </div>
    </template>
