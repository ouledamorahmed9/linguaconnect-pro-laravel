        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                @if($selectedTeacher)
                    ...
                    <!-- LIST VIEW (teacher-style cards) -->
                    <div x-show="viewMode === 'list'" class="p-6 md:p-8">
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <!-- Slots Grid (teacher-style cards grouped by day) -->
                            <div class="lg:col-span-2">
                                ...
                                @else
                                    @php
                                        ...
                                        // Rebuild a day-based grouping from existing grouped-by-client structure
                                        $byDay = collect();
                                        foreach ($weeklySlots as $clientName => $slots) {
                                            foreach ($slots as $slot) {
                                                $byDay[$slot->day_of_week] = ($byDay[$slot->day_of_week] ?? collect())->push($slot);
                                            }
                                        }
                                    @endphp
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        @foreach($daysOfWeek as $dayNumber => $dayName)
                                            @if(isset($byDay[$dayNumber]) && $byDay[$dayNumber]->count() > 0)
                                                <div class="space-y-4">
                                                    <h3 class="text-xl font-bold text-gray-800 border-b-2 border-indigo-500 pb-2">
                                                        {{ $dayName }}
                                                    </h3>
                                                    @foreach($byDay[$dayNumber] as $slot)
                                                        <div class="bg-white overflow-hidden shadow-sm rounded-lg border border-gray-200">
                                                            ...
                                                            <div class="bg-gray-50 px-5 py-3 border-t flex flex-wrap gap-2">
                                                                <a href="{{ route('admin.roster.edit', $slot) }}" class="px-4 py-2 bg-white text-indigo-700 border border-indigo-300 font-semibold rounded-lg text-sm hover:bg-indigo-50">
                                                                    تعديل الحصة
                                                                </a>
