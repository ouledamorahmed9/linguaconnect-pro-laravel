...
<div>
    <x-input-label for="bio" :value="__('نبذة تعريفية (Bio)')" />
...
<div class="mt-4">
    <x-input-label for="banner_photo" :value="__('صورة الغلاف (Banner)')" />
    <input id="banner_photo" name="banner_photo" type="file" class="mt-1 block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none">
    <p class="mt-1 text-xs text-gray-500">PNG, JPG up to 2MB</p>
    
    @if($user->banner_photo_path)
        <div class="mt-2">
            <img src="{{ $user->banner_url }}" class="h-20 w-full object-cover rounded-lg">
        </div>
    @endif
</div>
